...
public class MessageController implements CommunityConstant {
    ...
    public String getLetterList(Model model, Page page) {
        ...
        // 将会话列表中的每一个Message对象，查询一些额外的内容，再转成一个map集合，加入conversations会话列表，最终共享到model里面
        // 这样做的目的是，为原本pojo实体/表头，添加不存在的一些属性/字段，但是这些字段不方便在数据库存放
    ...
    public String getNoticeList(Model model) {
        ...
        if (message != null) {
            ...
            messageVO.put("user", userService.findUserById((Integer) data.get("userId"))); // 触发用户
            ...
            int unread = messageService.findNoticeUnreadCount(user.getId(), TOPIC_COMMENT); // 未读评论类通知总数
        ...
        if (message != null) {
            ...
            messageVO.put("entityType", data.get("entityType"));
            messageVO.put("entityId", data.get("entityId"));
            // 这里不需要指定postId（不用连接到帖子的查看详情而是用户的主页）

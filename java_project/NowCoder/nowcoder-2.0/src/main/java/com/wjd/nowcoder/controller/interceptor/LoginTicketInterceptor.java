...
public class LoginTicketInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (ticket != null) {
            ...
            // 检查凭证是否有效（不为null，且过期时间在当前时间之后，有效）
            if (loginTicket != null && loginTicket.getStatus() == 0 && loginTicket.getExpired().after(new Date())) {
                // Todo:凭证有效，在Redis中刷新过期时间
                // 根据凭证，查询用户pojo对象
                User user = userService.findUserById(loginTicket.getUserId());
                // 在本次请求中持有用户，同时线程隔离
                hostHolder.setUser(user);
                // SpringSecurity：构建用户认证的结果（通过密码）
                ...
                // 将认证的结果存入SecurityContext，以便于Security进行授权
    ...
    // 在渲染完毕后，将线程私有的User对象删除（下次访问页面时会再次拦截，创建User对象），并清除SecurityContext（认证结果）
    @Override
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) throws Exception {
        hostHolder.clear();
        // 清除SecurityContext（认证结果）
